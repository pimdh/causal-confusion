...
from ccil.utils.graph_distribution import (
    UniformDistribution,
    CombinatorialGumbelDistribution,
    sparse_prior_logits,
    GraphDistribution,
)
from ccil.utils.models import SimplePolicy, MLP, MaskPolicy, PolicyModel
from ccil.utils.policy_runner import (
    PolicyRunner,
    RandomMaskPolicyAgent,
    FixedMaskPolicyAgent,
)
...
def imitate(args):
    ...
    train_dataset, test_dataset = random_split(
        dataset, [args.num_samples, args.num_samples], args.data_seed
    )

    dataloaders = {
        "train": DataLoader(
            train_dataset, batch_size=64, shuffle=True, collate_fn=batch_cat
        ),
        "test": DataLoader(
            test_dataset, batch_size=64, shuffle=True, collate_fn=batch_cat
        ),
    }
    ...
    if args.network == "simple":
        ...
        graph_distribution = CombinatorialGumbelDistribution(
            0.5, 3, beta=0.05, prior_logits=prior_logits
        ).to(device)
        policy_model = MaskPolicy(MLP([6, 50, 50, 50, 3])).to(device)
        max_epochs = 20
    else:
    ...
    optimizer = torch.optim.Adam(
        list(policy_model.parameters()) + list(graph_distribution.parameters())
    )
    ...
    if args.save:
        name = (
            args.name
            or f"{args.input_mode}_{args.network}_{datetime.now():%Y%m%d-%H%M%S}"
        )
...
def main():
    ...
    parser.add_argument(
        "input_mode",
        choices=["original", "confounded"],
        help="Whether to add random action [original] or previous [confounded].",
    )
    parser.add_argument(
        "network",
        choices=["simple", "uniform", "combinatorial"],
        help=(
            "What kind of graph distribution to use. Simple: mask nothing, Uniform: sample uniformly. "
    ...
    parser.add_argument(
        "--data_seed",
        type=int,
        help="Seed for splitting train/test data. Default=random",
    )
    parser.add_argument(
        "--num_samples", type=int, default=300, help="Num samples used for training."
    )
    parser.add_argument(
        "--save", action="store_true", help="Whether to save learned policy."
    )
